...
object AssetPoolPolicies {
    ...
    fun canHold(authedUser: AuthedUserDTO, assetPool: city.smartb.registry.f2.pool.domain.model.AssetPoolDTO): Boolean = canTransitionAnd<AssetPoolHoldCommand>(assetPool) {
    ...
    fun canResume(authedUser: AuthedUserDTO, assetPool: city.smartb.registry.f2.pool.domain.model.AssetPoolDTO): Boolean = canTransitionAnd<AssetPoolResumeCommand>(assetPool) {
        authedUser.hasOneOfRoles(
            Roles.ORCHESTRATOR_ADMIN, Roles.ORCHESTRATOR_USER,
            Roles.PROJECT_MANAGER_ADMIN, Roles.PROJECT_MANAGER_USER
        )
    ...
    fun canClose(authedUser: AuthedUserDTO, assetPool: city.smartb.registry.f2.pool.domain.model.AssetPoolDTO): Boolean = canTransitionAnd<AssetPoolCloseCommand>(assetPool) {
        authedUser.hasOneOfRoles(
            Roles.ORCHESTRATOR_ADMIN, Roles.ORCHESTRATOR_USER,
            Roles.PROJECT_MANAGER_ADMIN, Roles.PROJECT_MANAGER_USER
        )
    ...
    private inline fun <reified C: AssetPoolCommand> canTransitionAnd(assetPool: city.smartb.registry.f2.pool.domain.model.AssetPoolDTO?, hasAccess: () -> Boolean): Boolean {
