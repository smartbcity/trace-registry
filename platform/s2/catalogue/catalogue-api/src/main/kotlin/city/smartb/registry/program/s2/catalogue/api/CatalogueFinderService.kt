...
class CatalogueFinderService(
	...
	override suspend fun getOrNull(id: CatalogueId): CatalogueModel? {
		return catalogueRepository.findById(id).orElse(null)?.toCatalogue(catalogueRepository)
	}

	override suspend fun getOrNullByIdentifier(id: CatalogueIdentifier): CatalogueModel? {
		return catalogueRepository.findByIdentifier(id).orElse(null)?.toCatalogue(catalogueRepository)
	}
	...
	override suspend fun page(
		...
		return cataloguePageQueryDB.execute(
			...
			it.toCatalogue(catalogueRepository)
		}
	}
}
