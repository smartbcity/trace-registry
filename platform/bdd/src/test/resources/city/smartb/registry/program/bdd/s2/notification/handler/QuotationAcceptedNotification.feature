Feature: QuotationAcceptedNotification
  ...
  Scenario: I want to send a notification when a beneficiary user accepts a quotation
    ...
    Then A notification should have been sent:
      | type                            | receivers | contact.firstname | contact.lastname | user.firstname | user.lastname | providerName | projectName | url.route                  | url.project |
      | BENEFICIARY__QUOTATION_ACCEPTED | bnf-admin | serge             | HANS             | ben            | EFICIAIRE     | p1           | projekt     | PROJECT_EQUIPMENT_FOLLOWUP | proj        |
